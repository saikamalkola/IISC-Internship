/*
 * PCA9685.c
 *
 *  Created on: 17-Jun-2018
 *      Author: kamal
 */

#include <stdbool.h>
#include <stdint.h>
#include "inc/tm4c123gh6pm.h"
#include "PCA9685.h"

void setRWMode(uint8_t mode)
{
    switch (mode)
    {
    case 0:
        I2C1_MSA_R &= ~(0x01);  //TX mode
        break;
    case 1:
        I2C1_MSA_R |= 0x01; //RX mode
        break;
    }
}

void waitAndErrorCheck()
{
    while (I2C1_MCS_R & 0x01)
        ; //Controller is busy..Wait here

    //Check for Error
    if (I2C1_MCS_R & 0x02)
    {
        //Error bit is set
        if (I2C1_MCS_R & 0x10)
        {
            //I2C Controller lost Arbitration
        }
        else
        {
            //I2C Controller won Arbitration
            I2C1_MCS_R = (1 << 2);  //Send STOP bit
            while (I2C1_MCS_R & 0x01)
                ;   //Controller is busy..Wait here
        }
    }
}

void wireSend(uint8_t byte, uint8_t condition)
{
    setRWMode(0);
    I2C1_MDR_R = byte;  //Write tx byte to the register
    I2C1_MCS_R = condition; //Configures START, RUN, STOP and DataAck bits

    /*
     *  DACK-STOP-START-RUN
     *    3    2    1    0
     */
    while (I2C1_MCS_R & 0x01)
        ; //Controller is busy..Wait here

    //Wait and Check for Error
    waitAndErrorCheck();
}

void wireRead(uint8_t address, uint8_t *data, uint8_t num_bytes)
{
    wireSend(address, (1 << 0) | (1 << 1));
    setRWMode(1);
    uint8_t i = 0;
    I2C1_MCS_R = ((1 << 0) | (1 << 1) | (1 << 3)); //Configures START, RUN, STOP and DataAck bits
    /*
     *  DACK-STOP-START-RUN
     *    3    2    1    0
     */
    for (i = 0; i < (num_bytes - 1); i++)
    {
        //Wait and Check for Error
        waitAndErrorCheck();
        data[i] = I2C1_MDR_R & 0xFF;
        I2C1_MCS_R = ((1 << 0) | (1 << 3)); //Configures START, RUN, STOP and DataAck bits
    }
    I2C1_MCS_R = ((1 << 0) | (1 << 2));
    waitAndErrorCheck();
    data[num_bytes - 1] = I2C1_MDR_R & 0xFF;
}

void I2CwriteByte(uint8_t Register, uint8_t Data)
{
    wireSend(Register, (1 << 0) | (1 << 1));
    wireSend(Data, (1 << 0) | (1 << 2));
}

void setSlaveAddress(uint8_t address)
{
    I2C1_MSA_R = (address << 1);
}

void PCA9685_digitalWrite(uint8_t pin, uint8_t state)
{
    switch(state)
    {
    case 1:
    {
        PCA9685_analogWrite(pin, 0, 4095);
    }
    break;
    case 0:
    {
        PCA9685_analogWrite(pin, 4095, 0);
    }
    break;
    }
}

void PCA9685_analogWrite(uint8_t pin, uint16_t on, uint16_t off)
{
    wireSend(LED0_ON_L + 4 * pin, (1 << 0) | (1 << 1));
    wireSend(on, (1 << 0));
    wireSend(on >> 8, (1 << 0));
    wireSend(off, (1 << 0));
    wireSend(off >> 8, (1 << 0) | (1 << 2));
}

void init_PCA9685()
{
    setSlaveAddress(PCA9685_ADDRESS);  //Setting slave address
    I2CwriteByte(MODE1_ADDRESS, 0x80);
    int i = 0;
    for (i = 0; i < 31800; i++)
        ;  //Wait for 10ms
    I2CwriteByte(MODE1_ADDRESS, 0x10);
    I2CwriteByte(PRESCALE_ADDRESS, PRESCALER_1526HZ);
    I2CwriteByte(MODE1_ADDRESS, 0xA0);
    for (i = 0; i < 31800; i++)
        ;  //Wait for 10ms
    //wireRead(MODE1_ADDRESS, &debug, 1);
    //wireRead(MODE1_ADDRESS, &debug, 1);
}

void init_I2C1()
{
    SYSCTL_RCGCI2C_R |= (0x02); //Enabling clock to I2C1 module
    SYSCTL_RCGC2_R |= 0x01; //Enable clock to port A
    GPIO_PORTA_LOCK_R = 0x4C4F434B; //Unlock Port A
    GPIO_PORTA_AFSEL_R |= ((1 << 6) | (1 << 7)); //Enabling Alternate Functionality for pins PA6 and PA7
    GPIO_PORTA_DEN_R |= ((1 << 6) | (1 << 7)); //Enabling Digital functionality of pins PA6 and PA7
    GPIO_PORTA_ODR_R |= (1 << 7);   //Enabling Open drain for SDA pin
    GPIO_PORTA_PUR_R |= ((1 << 6) | (1 << 7));
    GPIO_PORTA_PCTL_R &= ~((0xF << 24) | (0xF << 28));
    GPIO_PORTA_PCTL_R |= ((3 << 24) | (3 << 28)); //Configuring I2C PA6 and PA7 as I2C pins

    I2C1_MCR_R |= (1 << 4); //Configuring I2C1 module as MASTER
    I2C1_MTPR_R = 0x01; //Configuring SCL of I2C1 to run at 400KHz
}
